...
if st.session_state.start_time:
    ...
    # Show time remaining in sidebar (even though hidden, it's tracked)
...
# 2-Column Layout to fit on one screen
col_left, col_right = st.columns([3, 2])

with col_left:
    # PROOFREAD INTRO (Your exact words, minor grammar fixes only)
    st.markdown("""
    ...
    # PROOFREAD INVITATION (Your exact words)
    st.markdown("""
    <div class="vip-box">
...
with col_right:
    ...
    with c2:
        if st.button("⏹️ STOP", use_container_width=True, key="stop_btn"):
            st.session_state.recording = False
            st.session_state.transcript = "Sequel of Mission Impossible coming soon. Who supporting male actor this time."
            st.rerun()
    ...
    if st.session_state.transcript:
        ...
        <div class="history-box" style="border-left-color: #2ecc71;">
            ...
            <p style="color:#fff; margin:0; font-size:0.9rem;">Sequel to Mission Impossible is coming soon. Who will be the supporting male actor this time?</p>
        </div>
        """, unsafe_allow_html=True)
