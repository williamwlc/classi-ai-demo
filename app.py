...
if not st.session_state.authenticated:
    ...
    st.stop()   # do not show the rest until authenticated
...
# ---------- Main App (only after authentication) ----------
...
# ---------- Record Your Sentence Section ----------
st.markdown("### 🎤 Record your sentence")
st.caption("*Recommended recording length: at least 5 seconds*")
...
with col_btn2:
    if st.session_state.recording:
        audio_bytes = st.audio_input("Recording...", label_visibility="collapsed")
...
gt_text = st.text_area(
    "📝 Ground truth (what you said)",
    placeholder="e.g., I like to read books",
    height=100,
)
...
if st.button("Submit & Analyze", type="primary", disabled=st.session_state.audio_bytes is None or not gt_text):
    audio_bytes = st.session_state.audio_bytes
    audio_bytes.seek(0)
    audio_data = audio_bytes.read()

    # Optional local duration check
    try:
        audio, sr = librosa.load(io.BytesIO(audio_data), sr=16000, mono=True)
        if len(audio) < 5 * sr:
            st.warning("Recording is shorter than 5 seconds. Please record a longer sentence.")
            st.stop()
...
if st.session_state.history:
    ...
    for entry in reversed(st.session_state.history):
        with st.expander(f"{entry['time']} | Carbon WER: {entry['carbon_wer']:.4f}" if entry['carbon_wer'] is not None else entry['time']):
    ...
    if st.session_state.recording_count >= 5:
        ...
        if carbon_wers:
            ...
            <div style="background:#0b3d91;padding:1.5rem;border-radius:12px;text-align:center;">
                ...
                <p style="font-size:2.5rem;font-weight:bold;color:#ffd966;">{avg:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
        else:
