...
# Text Input Box
st.markdown("---")
st.markdown("### 📝 Please type or paste your English text now or after your recording.")
...
# WER Score placeholder
recording_count = st.session_state.get("recording_count", 0)

if recording_count >= 5:
    st.markdown(f"""
    <div class="wer-box">
        <div><strong>📊 Word Error Rate</strong></div>
        <div class="wer-score">—</div>
        <div class="wer-note">WER score will update after 5 recordings.</div>
    </div>
    """, unsafe_allow_html=True)
else:
    st.markdown(f"""
